...
fun waitUntilViewIsDisplayedById(id: Int) {
    waitUntilViewIsDisplayed(withId(id))
}

fun waitUntilViewIsDisplayedByText(text: String) {
    waitUntilViewIsDisplayed(allOf(withText(text)))
}

fun waitUntilViewIsDisplayed(matcher: Matcher<View?>) {
    val idlingResource: IdlingResource =
        ViewIdlingResource(matcher, isDisplayed())
    try {
